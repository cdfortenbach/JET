...
* The JET code was written in Python 3.6.1/2.  It is highly recommended that you use Python 3.6.1/2 to run JET.
...
* Many of the required packages (unless they have < pip > in the build column of Table 2) can then be installed from the Linux command line with:
 ...
 ```conda install -c conda-forge astropy=2.0.2=np113py36_1```
 
 or another example:
 
 ```conda install -c http://ssb.stsci.edu/astroconda photutils=0.4=py36_1```
...
|-----------------------|-------------------|----------------------------------------|-------------------------------------|
| astropy               | Python pkg.       | 2.0.2 / np113py36_1                    | conda-forge                         |
| bokeh                 | Python pkg.       | 0.12.6 / py36_0                        | conda-forge                         |
| joblib                | Python pkg.       | 0.11 / py36_0                          |                                     |
| matplotlib            | Python pkg.       | 2.0.2 / np113py36_0                    |                                     |
| numpy                 | Python pkg.       | 1.14.0 / < pip >                       |                                     |
| pandeia.engine        | Python pkg.       | 1.4 / < pip >                          |                                     |
| pandexo.engine        | Python pkg.       | 1.3 / < pip >                          |                                     |
| photutils             | Python pkg.       | 0.4 / py36_1                           | http://ssb.stsci.edu/astroconda     |
| pyephem               | Python pkg.       | 3.7.6.0 / < pip >                      |                                     |
| pyfftw                | Python pkg.       | 0.10.4 / py36_3                        | conda-forge                         |
...
| spectres              | Python module     | 2.0.0 / < pip >                        |                                     |
| sphinx                | Python pkg.       | 1.5.6/ py36_0                          | conda-forge                         |
| synphot               | Python pkg.       | 0.1 / py36_1                           | http://ssb.stsci.edu/astroconda     |
...
| ExoT_Master.58.py           | Python program (element of JET)                                                          |
| ExoT_USR_MANUAL.txt         | USR_MANUAL for Exo-Transmit                                                              |
| JET_Input.txt               | file for primary input to JET                                                            |
| JET_Master.55.sh            | Linux shell script controlling the JET code                                              |
...
| Pdxo_Master.67.py           | Python program (element of JET)                                                          |
| Pdxo_Output.txt             | internal data transfer file                                                              |
| Rank_Master.10.py           | Python program (element of JET)                                                          |
...
3.  Using a .txt editor (or development environment), open the **JET_Input.txt** file and make the appropriate changes to the input parameters.  The following shows an example set-up for a run with NIRSpec G395M.  Only the boldface parameters should be changed.  Only make changes to User Input values in rows 7 - 45, and cols 60 - 79!!   Do not change the syntax for any item.  This is a comma delimited file.  Commas should only be included where indicated.
...
| Targ Acq: target acquisition if any (sec):               |, **492**| 
| Exposure Ovhd: factor 1:                                 |, **0.0393**| 
| Exposure Ovhd: factor 2 (sec):                           |, **26**| 
| Mech: mechanism movements (sec):                         |, **110**| 
| OSS: Onboard Script System compilation (sec):            |, **65**| 
| MSA: NIRSpec MSA configuration (sec):                    |, **0**| 
| IRS2: NIRSpec IRS2 Detector Mode setup (sec):            |, **0**| 
| Visit Ovhd: visit cleanup activities (sec):              |, **110**| 
| Obs Ovhd factor (%):                                     |, **16**| 
| DS Ovhd (sec):                                           |, **0**|        
|                                                          |                           |
| RunExoT (Y/N):                                           |, **Y**| 
 ...
 * The spectral resolving power (R value = Res) should be set.  Currently we have only tested Res = 100. The model spectra are initially generated with a resolving power of ~ 1000.  For the statistical analysis and for plotting purposes the model spectra and simulated spectra are binned down to the same resolving power.  The factor, Res, sets this value.  This is generally well below the native resolving power of the instrument/modes studied.
...
* After a few seconds, you should see . . . 
 ...
 Elapsed time for ExoT_Master (sec):  324.8202679157257Y
...
```dos
 ...
 Transferring planetary system data and model spectra to Pandexo . . .
      ...
      Generating simulated instrument spectra for target:  6 
         ...
         Comparing simulation to model spectrum and flat line for hi_metal atm: 
               ...
               min # of transits for mean dBIC (less one sigma) > 10 :  44
 ...
 Elapsed time for Pdxo_Master (sec):  292.81097984313965
...
for JET: 
Fortenbach, C. D., & Dressing, C. D. (in prep)
